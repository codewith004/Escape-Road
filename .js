...
// --- Sounds with graceful handling ---
const jumpSound = new Audio('jump.mp3');
const powerUpSound = new Audio('powerup.mp3');
const gameOverSound = new Audio('gameover.mp3');

// Warn if audio fails to load but don't break the game
[jumpSound, powerUpSound, gameOverSound].forEach((s) => {
    s.addEventListener('error', () => {
        console.warn('Audio failed to load:', s.src);
    });
});

// Safely wrap/override Player methods to add double-jump and sound without modifying the class directly
const originalPlayerUpdate = Player.prototype.update;
const originalPlayerJump = Player.prototype.jump;

// Reset doubleJumped flag when player lands
...
// Extend jump to support double jump and play sound (with promise rejection handling)
Player.prototype.jump = function() {
    if (this.isGrounded) {
        ...
        this.velocityY = this.game.physics.jumpForce * 0.8;
        this.isJumping = true;
...
// Wrap Game.activatePowerUp and Game.endGame to play sounds when appropriate
const originalActivatePowerUp = Game.prototype.activatePowerUp;
Game.prototype.activatePowerUp = function(powerUp) {
    // play sound, but don't let failures break game logic
    powerUpSound.play().catch(() => {});
    originalActivatePowerUp.call(this, powerUp);
};
...
// --- Pause Feature ---
// Make pause toggle more robust and ensure the loop respects the isPaused flag
document.addEventListener('keydown', (event) => {
    if (event.code === 'KeyP') {
        myGame.isPaused = !myGame.isPaused;
        if (!myGame.isPaused) {
            // resume the loop using a proper timestamp
            myGame.lastUpdateTime = performance.now();
            myGame.gameLoop(myGame.lastUpdateTime);
        }
    }
});

// Wrap the gameLoop to respect isPaused without altering the original implementation
const originalGameLoop = Game.prototype.gameLoop;
Game.prototype.gameLoop = function(timestamp) {
    if (this.isPaused) {
        // keep updating lastUpdateTime so deltaTime doesn't spike when unpausing
        this.lastUpdateTime = timestamp;
        requestAnimationFrame(this.gameLoop.bind(this));
        return;
    }
...
// Ensure the instance has an isPaused flag
